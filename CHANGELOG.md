...
- **Upload API** completed: `explicit`, `rename`, tag and context commands, structured metadata updates, `explode`, `multi`, `generateSprite`, `text`, archives and `deleteByToken`.
...
- `upload.deleteByToken` posted to `/<resource_type>/delete_by_token`, which answers 404 with an HTML body and leaves the asset in place. The route sits directly under the cloud name, so the method no longer takes a resource type. Verified end to end: an unsigned upload's token now deletes its asset.
- `upload.destroyByAssetId` is gone. Cloudinary serves no asset-ID destroy on the Upload API: `DELETE /v1_1/<cloud>/asset/<id>` answers 404 with an HTML body, and `POST /<type>/destroy` rejects `asset_id` with "Missing required parameter - public_id", leaving the asset in place. Use `admin.resources.deleteByAssetIds`, which is verified against a live cloud.
