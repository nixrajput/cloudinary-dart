...
- `admin.streamingProfiles.create` and `update` took `representations` as optional. Cloudinary requires it on both, so omitting it produced a request that could only answer 400. Both now require it.
- `upload.destroyByAssetId` targeted `DELETE /v1_1/<cloud>/asset/<id>`, a route that does not exist and answers 404. It now posts `asset_id` to the `destroy` endpoint, which is the shape the Upload API reference documents.
